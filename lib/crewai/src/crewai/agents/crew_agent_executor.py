    ...
    def _invoke_loop(self) -> AgentFinish:
        ...
        while not isinstance(formatted_answer, AgentFinish):
            try:
                ...
                print(f"DEBUG: Before final message append, message count: {len(self.messages)}")
                for i, msg in enumerate(self.messages):
                    print(f"DEBUG:   [{i}] role={msg.get('role')}, content={msg.get('content', '')[:100]}")
                
                if isinstance(formatted_answer, AgentFinish):
                    print(f"DEBUG: Appending final answer as assistant message")
                    self._append_message(formatted_answer.text)
                else:
                    print(f"DEBUG: Not appending, formatted_answer is AgentAction")
                
                print(f"DEBUG: After final message append, message count: {len(self.messages)}")
                for i, msg in enumerate(self.messages):
                    print(f"DEBUG:   [{i}] role={msg.get('role')}, content={msg.get('content', '')[:100]}")
    ...
    async def _ainvoke_loop(self) -> AgentFinish:
        ...
        while not isinstance(formatted_answer, AgentFinish):
            try:
                ...
                print(f"DEBUG: About to call LLM (async) with {len(self.messages)} messages:")
                for i, msg in enumerate(self.messages):
                    print(f"DEBUG:   [{i}] role={msg.get('role')}, content={msg.get('content', '')[:100]}")
                ...
                # The tool result has already been added as a tool message in _handle_agent_action
                # Only append the formatted_answer text if it's a final answer (AgentFinish)
                print(f"DEBUG: Before final message append (async), message count: {len(self.messages)}")
                for i, msg in enumerate(self.messages):
                    print(f"DEBUG:   [{i}] role={msg.get('role')}, content={msg.get('content', '')[:100]}")
                
                if isinstance(formatted_answer, AgentFinish):
                    print(f"DEBUG: Appending final answer as assistant message (async)")
                    self._append_message(formatted_answer.text)
                else:
                    print(f"DEBUG: Not appending (async), formatted_answer is AgentAction")
                
                print(f"DEBUG: After final message append (async), message count: {len(self.messages)}")
                for i, msg in enumerate(self.messages):
                    print(f"DEBUG:   [{i}] role={msg.get('role')}, content={msg.get('content', '')[:100]}")
